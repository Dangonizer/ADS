...
{
	bool delOK = false;
	if (validateVertex(e.either()) && validateVertex(e.other(e.either())))
	{
		...
		for (int i = 0; i < adj[e.other(e.either())].size(); i++)
		{
			if (adj[e.other(e.either())][i] == e)
			{
				adj[e.other(e.either())].erase(adj[e.other(e.either())].begin() + i);
				// break;
			}
		}
		// E--;
